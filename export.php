...
if (!isset($queries[$type])) { http_response_code(404); exit('Export tidak ditemukan.'); }
[$filename, $header, $sql] = $queries[$type]; header('Content-Type: text/csv; charset=utf-8'); header('Content-Disposition: attachment; filename=' . $filename);
$out = fopen('php://output', 'w'); fputcsv($out, $header); foreach (query_all($sql) as $row) fputcsv($out, array_values($row)); fclose($out);
