...
<head>
    ...
    <title>Masuk · HPP Smart Pricing</title>
...
<body>
    <main class="container auth-page">
        <section class="panel auth-card">
            <p class="eyebrow"><?= $hasUsers ? 'AKSES APLIKASI' : 'PENYIAPAN AWAL' ?></p>
            <h1><?= $hasUsers ? 'Masuk ke aplikasi' : 'Buat akun admin' ?></h1>
            <p class="muted">
                <?= $hasUsers ? 'Gunakan akun terdaftar untuk melanjutkan.' : 'Masukkan setup key dari konfigurasi server untuk membuat admin pertama.' ?>
            </p>
            <?php if ($error !== ''): ?>
            ...
            <form method="post"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><input type="hidden"
                    name="next" value="<?= e($next) ?>"><?php if (!$hasUsers): ?><label>Setup key<input name="setup_key"
                            autocomplete="off" required></label><?php endif; ?><label>Username<input name="username"
                        autocomplete="username" required></label><label>Password<input type="password" name="password"
                        autocomplete="<?= $hasUsers ? 'current-password' : 'new-password' ?>" minlength="8"
                        required></label><button class="button primary full"
                    type="submit"><?= $hasUsers ? 'Masuk' : 'Buat akun dan masuk' ?></button></form>
        </section>
    </main>
</body>
