...
<div class="workspace">
    <section class="panel" id="recipe-form">
        ...
        <form method="post" action="products.php" id="form-product"><?= csrf_field() ?>
            ...
            <label>Nama produk
                <input name="recipe_name" required value="<?= e($editRecipe['name'] ?? '') ?>" placeholder="Es Kopi Susu">
            </label>
            ...
            <div class="form-grid compact">
                ...
                <label>Biaya alat
                    <input 
                        ...
                        oninput="formatRupiah(this); calculateLiveHpp();"
                        placeholder="Rp 0"
                    >
                </label>
                ...
                <label>Estimasi HPP
                    <input id="display-hpp" type="text" value="Rp 0" readonly style="background-color: #f3f4f6; font-weight: 600; color: #4b5563;">
                </label>
                <label>Harga Rekomendasi
                    <input id="display-recommended-price" type="text" value="Rp 0" readonly style="background-color: #f3f4f6; font-weight: 700; color: #2563eb;">
                </label>
                ...
                <div style="grid-column: span 2; margin-top: 10px;">
                    ...
                    <div style="display: flex; gap: 20px; align-items: center; margin-bottom: 8px;">
                        <label style="display: inline-flex; align-items: center; gap: 6px; cursor: pointer;">
                            <input type="radio" name="price_mode" value="auto" id="mode-auto" onchange="togglePriceMode()" <?= !$isManualPrice ? 'checked' : '' ?>>
                            Gunakan Rekomendasi Tier
                        </label>
                        <label style="display: inline-flex; align-items: center; gap: 6px; cursor: pointer;">
                            <input type="radio" name="price_mode" value="manual" id="mode-manual" onchange="togglePriceMode()" <?= $isManualPrice ? 'checked' : '' ?>>
                            Input Manual
                        </label>
                    </div>
                </div>
            ...
            <table id="product-table">
                ...
                <tbody>
                    <?php foreach ($recipes as $r): ?>
                        <tr>
                            <td><strong><?= e($r['name']) ?></strong><small>Markup <?= e($r['price_tier']) ?></small></td>
                            <td><?= rupiah($r['hpp_live']) ?></td>
                            <td><?= rupiah($r['recommended_price'] ?? 0) ?></td>
                            <td><strong><?= rupiah($r['selling_price']) ?></strong></td>
                            <td><a class="button secondary small" href="?edit=<?= $r['id'] ?>">Edit</a> <a
                                    class="button secondary small" href="pos.php?product=<?= $r['id'] ?>">Jual</a>
                                <form method="post" style="display:inline" onsubmit="return confirm('Nonaktifkan produk?')">
                                    <?= csrf_field() ?><input type="hidden" name="action" value="delete_recipe"><input
                                        type="hidden" name="recipe_id" value="<?= $r['id'] ?>"><button
                                        class="button danger small">Nonaktifkan</button></form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
...
<template id="ingredient-line-template">
    <div class="ingredient-line">
        ...
        <label>Jumlah
            <input name="quantity[]" class="input-ingredient-qty" type="number" step="any" min="0.001" placeholder="1" oninput="calculateLiveHpp()" required>
        </label>
        <label>Satuan
            <select name="unit[]" class="input-ingredient-unit" onchange="calculateLiveHpp()">
                <?php foreach (array_keys(APP_UNITS) as $u): ?>
