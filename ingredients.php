...
<section class="panel" id="ingredient-form">
    ...
    <form method="post" action="ingredients.php" class="form-grid" id="form-ingredient"><?= csrf_field() ?><input type="hidden" name="action"
            value="<?= $edit ? 'update_ingredient' : 'add_ingredient' ?>"><input type="hidden" name="return_to"
            value="ingredients.php"><?php if ($edit): ?><input type="hidden" name="ingredient_id"
                value="<?= $edit['id'] ?>"><?php endif; ?>
        ...
        <label>Harga total pembelian
            <input 
                ...
                oninput="formatRupiah(this); hitungHPP();"
                placeholder="Rp 0"
                required
            >
        </label>
        ...
        <label>Stok minimum
            <input name="min_stock" type="number" min="0" step="any" value="<?= e(isset($edit['min_stock']) ? (float)$edit['min_stock'] : '0') ?>">
        </label>
        
        <!-- Stok Maksimum (Menggunakan (float)) -->
        <label>Stok maksimum
            <input name="max_stock" type="number" min="0" step="any" value="<?= e(isset($edit['max_stock']) ? (float)$edit['max_stock'] : '0') ?>">
        </label>
